...
export function buildWorkspaceStatus(now = Date.now()) {
  ...
  for (const session of allParsed()) {
    ...
    if (!item) {
      item = { key, name, lastAt: 0, sessions: 0, activeEvents: 0, todayEvents: 0, todayTokens: 0, latestSession: null }
...
async function pushWorkspaceStatus() {
  ...
  try {
    const body = JSON.stringify(buildWorkspaceStatus())
...
export function apply(ctx) {
  ...
  void pushWorkspaceStatus()
  const pushInterval = setInterval(() => { void pushWorkspaceStatus() }, dashboardConfig().intervalSeconds * 1000)
