  ...
  pricingSyncPromise = (async () => {
    ...
    try {
      const response = await fetch(baseUrl + '/api/v1/channels/available', {
        headers: { Authorization: 'Bearer ' + state.config.apiKey, 'X-API-Key': state.config.apiKey, Accept: 'application/json' },
      ...
      if (!response.ok || (json && json.code !== undefined && Number(json.code) !== 0)) {
        throw new Error(String((json && (json.message || json.error)) || ('HTTP ' + response.status)))
      }
