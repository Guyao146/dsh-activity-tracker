...
const emptyPricingState = () => ({ config: { baseUrl: '', apiKey: '', refreshToken: '', expiresAt: 0, user: null, groupId: '' }, snapshots: {}, lastSync: null, error: null })
...
function publicPricingState(state) {
  ...
  return { config: { baseUrl: state.config.baseUrl, configured: Boolean(state.config.baseUrl && token), loggedIn: Boolean(state.config.refreshToken), credentialType, user: state.config.user || null, expiresAt: Number(state.config.expiresAt) || 0, groupId: state.config.groupId || '' }, snapshots: state.snapshots, lastSync: state.lastSync, error: state.error }
}
...
function saveTokenPair(state, data) {
  ...
  if (data.user) state.config.user = { id: data.user.id, email: data.user.email || '', username: data.user.username || data.user.name || '', role: data.user.role || '' }
  state.error = null
...
async function loginSub2API(baseUrlValue, email, password) {
  ...
  return { ok: true, requires2FA: false, ...publicPricingState(state) }
}
...
async function syncPricing(force = false) {
  ...
  pricingSyncPromise = (async () => {
    ...
    try {
      let got = await sub2Request(baseUrl, '/api/v1/channels/available', { headers: { Authorization: 'Bearer ' + token } })
      if (got.response.status === 401 && state.config.refreshToken) {
        token = await refreshSub2APIToken(state)
        got = await sub2Request(baseUrl, '/api/v1/channels/available', { headers: { Authorization: 'Bearer ' + token } })
      }
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (url.pathname === API + '/pricing/config') {
          ...
          if (body.clearKey === true) { state.config.apiKey = ''; state.config.refreshToken = ''; state.config.expiresAt = 0; state.config.user = null }
