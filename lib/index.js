...
const dashboardFile = () => join(dshHome(), 'dsh-activity-tracker-dashboard.json')
...
function dashboardConfig() {
  let file = {}
  try { file = JSON.parse(readFileSync(dashboardFile(), 'utf8').replace(/^\uFEFF/, '')) || {} } catch (e) { file = {} }
  const runtime = String(process.env.DSH_ACTIVITY_DASHBOARD_TOKEN || '').trim()
  const configured = file.authorizedWorkspaces
  return {
    token: runtime || String(file.token || '').trim(),
    pushUrl: String(process.env.DSH_ACTIVITY_DASHBOARD_PUSH_URL || file.pushUrl || '').trim(),
    intervalSeconds: Math.max(10, Math.min(300, Number(file.intervalSeconds) || 10)),
    authorizedWorkspaces: configured === '*' ? '*' : (Array.isArray(configured) ? configured.filter((key) => /^[a-f0-9]{16}$/.test(String(key))).slice(0, 500) : []),
  }
}
function writeDashboardConfig(next) {
  const current = (() => { try { return JSON.parse(readFileSync(dashboardFile(), 'utf8').replace(/^\uFEFF/, '')) || {} } catch (e) { return {} } })()
  const value = {
    ...current,
    token: String(next.token || current.token || '').trim(),
    pushUrl: String(next.pushUrl || current.pushUrl || '').trim(),
    intervalSeconds: Math.max(10, Math.min(300, Number(next.intervalSeconds || current.intervalSeconds) || 10)),
    authorizedWorkspaces: next.authorizedWorkspaces === '*' ? '*' : (Array.isArray(next.authorizedWorkspaces) ? next.authorizedWorkspaces : (Array.isArray(current.authorizedWorkspaces) ? current.authorizedWorkspaces : [])),
  }
  mkdirSync(dshHome(), { recursive: true })
  writeFileSync(dashboardFile(), JSON.stringify(value, null, 2), { encoding: 'utf8', mode: 0o600 })
  return value
}
async function pairDashboard(baseUrlValue, code, allowDetails) {
  let target
  try { target = new URL(String(baseUrlValue || '').trim()) } catch (e) { throw new Error('生活看板地址无效') }
  if (target.protocol !== 'https:' || target.username || target.password) throw new Error('生活看板地址必须是 HTTPS 地址')
  target.searchParams.set('action', 'workspace-pair-consume')
  const response = await fetch(target, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ code }), signal: AbortSignal.timeout(10000) })
  const data = await response.json().catch(() => null)
  if (!response.ok || !data?.ok || typeof data.token !== 'string' || data.token.length < 32) throw new Error(data?.error || `看板配对失败（HTTP ${response.status}）`)
  const pushUrl = new URL(target)
  pushUrl.searchParams.set('action', 'workspace-push')
  const saved = writeDashboardConfig({ token: data.token, pushUrl: pushUrl.toString(), intervalSeconds: Number(data.intervalSeconds) || 10, authorizedWorkspaces: allowDetails ? '*' : [] })
  return { ok: true, paired: true, pushUrl: saved.pushUrl, authorizedWorkspaces: saved.authorizedWorkspaces }
}
...
function parseSession(id, text) {
  const info = { id, cwd: '', createdAt: 0, title: '', turns: 0, events: [], tokens: [], records: [] }
  for (const line of text.split('\n')) {
    ...
    if (j.type === 'session') {
      ...
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'user', text: label.slice(0, 12000) })
    } else if (j.type === 'tool/call') {
      const name = String(d.name || '')
      info.events.push({ t, k: kindOfTool(name), label: toolLabel(name, d.arguments), tool: name })
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'tool', name: name.slice(0, 120), text: toolLabel(name, d.arguments).slice(0, 1000), status: 'running' })
      ...
      if (text) info.records.push({ seq: Number(j.seq) || 0, t, type: 'assistant', text: String(text).slice(0, 24000), model: String(src.model || '').slice(0, 120) })
    } else if (j.type === 'tool/result') {
      const message = typeof d.message === 'string' ? d.message : (d.message ? JSON.stringify(d.message) : '')
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'tool-result', text: message.slice(0, 8000), status: d.error ? 'error' : 'done' })
    } else if (j.type === 'approval/asked') {
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'approval', name: String(d.toolName || '').slice(0, 120), text: String(d.reason || '').slice(0, 2000), status: 'pending' })
    } else if (j.type === 'approval/decided') {
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'approval-result', text: String(d.outcome || '').slice(0, 120), status: String(d.outcome || '').slice(0, 40) })
    } else if (j.type === 'turn/end') {
      info.records.push({ seq: Number(j.seq) || 0, t, type: 'turn-end', text: String(d.reason || '').slice(0, 240) })
    }
  }
  return info
}
...
// ---------------- 生活看板：实时工作区状态 ----------------
const WORKSPACE_THRESHOLDS = Object.freeze({ working: 2 * 60_000, active: 15 * 60_000, recent: 60 * 60_000 })
function workspaceState(lastAt, now) {
  if (!lastAt) return 'none'
  const age = Math.max(0, now - lastAt)
  if (age <= WORKSPACE_THRESHOLDS.working) return 'working'
  if (age <= WORKSPACE_THRESHOLDS.active) return 'active'
  if (age <= WORKSPACE_THRESHOLDS.recent) return 'recent'
  return 'idle'
}
...
export function buildWorkspaceStatus(now = Date.now()) {
  const today = dateKey(now)
  const activeCutoff = now - WORKSPACE_THRESHOLDS.active
  const projects = new Map()
  const permissions = dashboardConfig().authorizedWorkspaces
  for (const session of allParsed()) {
    const pathKey = String(session.cwd || 'unknown').toLowerCase()
    const key = createHash('sha256').update(pathKey).digest('hex').slice(0, 16)
    const name = projNameOf(session.cwd)
    let item = projects.get(pathKey)
    if (!item) {
      item = { key, name, lastAt: 0, sessions: 0, activeEvents: 0, todayEvents: 0, todayTokens: 0, latestSession: null, detailSessions: [] }
      projects.set(pathKey, item)
    }
    item.sessions++
    const allowed = permissions === '*' || permissions.includes(key)
    if (allowed) item.detailSessions.push({
      id: session.id.replace(/^session-/, ''), title: session.title.slice(0, 240), createdAt: session.createdAt || 0,
      turns: session.turns || 0, records: session.records.slice(-120),
    })
    let sessionLastAt = session.createdAt || 0
    for (const event of session.events) {
      if (event.t > sessionLastAt) sessionLastAt = event.t
      if (event.t >= activeCutoff) item.activeEvents++
      if (event.t && dayHour(event.t)[0] === today) item.todayEvents++
    }
    for (const token of session.tokens) {
      if (token.t > sessionLastAt) sessionLastAt = token.t
      if (token.t && dayHour(token.t)[0] === today) item.todayTokens += token.in + token.out + token.cache
    }
    if (sessionLastAt > item.lastAt) {
      item.lastAt = sessionLastAt
      item.latestSession = {
        id: session.id.replace(/^session-/, '').slice(0, 8),
        turns: session.turns || 0,
        lastAt: sessionLastAt,
      }
    }
  }
  const workspaces = [...projects.values()].map((item) => ({
    ...item,
    detailAuthorized: permissions === '*' || permissions.includes(item.key),
    detailSessions: item.detailSessions.sort((a, b) => (b.records.at(-1)?.t || b.createdAt) - (a.records.at(-1)?.t || a.createdAt)).slice(0, 20),
    state: workspaceState(item.lastAt, now),
    ageMs: item.lastAt ? Math.max(0, now - item.lastAt) : null,
  })).sort((a, b) => b.lastAt - a.lastAt || a.name.localeCompare(b.name))
  return {
    ok: true,
    generatedAt: now,
    tz: Intl.DateTimeFormat().resolvedOptions().timeZone || '',
    thresholdsMs: WORKSPACE_THRESHOLDS,
    summary: {
      total: workspaces.length,
      working: workspaces.filter((item) => item.state === 'working').length,
      active: workspaces.filter((item) => item.state === 'active').length,
      recent: workspaces.filter((item) => item.state === 'recent').length,
      idle: workspaces.filter((item) => item.state === 'idle').length,
      none: workspaces.filter((item) => item.state === 'none').length,
    },
    workspaces,
  }
}

function localSessionId(exposedId) {
  return allParsed().find((session) => session.id.replace(/^session-/, '') === exposedId)?.id || ''
}

let workspacePushRunning = false
let workspacePushFailures = 0
const processedRemoteCommands = new Set()
export async function deliverRemoteCommands(ctx, commands) {
  for (const command of Array.isArray(commands) ? commands : []) {
    if (!command || processedRemoteCommands.has(command.id) || !/^[a-f0-9-]{36}$/i.test(String(command.id))) continue
    const status = buildWorkspaceStatus(), workspace = status.workspaces.find((item) => item.key === command.workspaceKey)
    const session = workspace?.detailSessions.find((item) => item.id === command.sessionId)
    if (!workspace?.detailAuthorized || !session || typeof command.message !== 'string' || !command.message.trim()) continue
    const fullId = localSessionId(session.id)
    if (!fullId) continue
    const agent = ctx.agents?.get(fullId)
    if (!agent?.followup) throw new Error('目标会话当前未在 DSH 中运行，请先在 DSH 打开该会话')
    agent.followup({ id: randomUUID(), role: 'user', content: [{ type: 'text', text: command.message.trim().slice(0, 8000) }], source: { kind: 'user' } })
    processedRemoteCommands.add(command.id)
    if (processedRemoteCommands.size > 1000) processedRemoteCommands.delete(processedRemoteCommands.values().next().value)
  }
}
function proxyFor(target) {
  const noProxy = String(process.env.NO_PROXY || process.env.no_proxy || '').split(',').map((item) => item.trim().toLowerCase()).filter(Boolean)
  const host = target.hostname.toLowerCase()
  if (noProxy.some((rule) => rule === '*' || host === rule.replace(/^\./, '') || host.endsWith(rule.startsWith('.') ? rule : '.' + rule))) return null
  const value = String(process.env.HTTPS_PROXY || process.env.https_proxy || process.env.ALL_PROXY || process.env.all_proxy || '').trim()
  if (!value) return null
  try { return new URL(value) } catch (e) { return null }
}
function postWorkspaceSnapshot(target, headers, body) {
  const proxy = proxyFor(target)
  if (!proxy) return fetch(target, { method: 'POST', headers, body, signal: AbortSignal.timeout(8000) })
  if (proxy.protocol !== 'http:') return Promise.reject(new Error('工作区推送当前只支持 HTTP CONNECT 代理'))
  return new Promise((resolve, reject) => {
    const connect = http.request({
      ...
      headers: proxy.username ? { 'Proxy-Authorization': 'Basic ' + Buffer.from(decodeURIComponent(proxy.username) + ':' + decodeURIComponent(proxy.password)).toString('base64') } : {},
    })
    connect.setTimeout(8000, () => connect.destroy(new Error('代理连接超时')))
    connect.on('error', reject)
    connect.on('connect', (response, socket) => {
      if (response.statusCode !== 200) { socket.destroy(); reject(new Error('代理 CONNECT 返回 HTTP ' + response.statusCode)); return }
      const secure = tls.connect({ socket, servername: target.hostname, rejectUnauthorized: true })
      secure.setTimeout(8000, () => secure.destroy(new Error('TLS 连接超时')))
      secure.on('error', reject)
      secure.once('secureConnect', () => {
        const request = https.request({ hostname: target.hostname, port: target.port || 443, path: target.pathname + target.search, method: 'POST', headers, agent: false, createConnection: () => secure }, (res) => {
          const chunks = []
        ...
        request.setTimeout(8000, () => request.destroy(new Error('推送请求超时')))
...
async function pushWorkspaceStatus(ctx) {
  if (workspacePushRunning) return
  const config = dashboardConfig()
  if (!config.pushUrl) return
  if (config.token.length < 32) {
    if (workspacePushFailures++ === 0) console.warn('[dsh-activity-tracker] 工作区推送令牌至少需要 32 个字符')
    return
  }
  let target
  try { target = new URL(config.pushUrl) } catch (e) {
    if (workspacePushFailures++ === 0) console.warn('[dsh-activity-tracker] 工作区推送地址无效')
    return
  }
  if (target.protocol !== 'https:' || target.username || target.password) {
    if (workspacePushFailures++ === 0) console.warn('[dsh-activity-tracker] 工作区推送地址必须是无账号密码的 HTTPS 地址')
    return
  }
  workspacePushRunning = true
  try {
    const snapshot = buildWorkspaceStatus()
    snapshot.commandAcks = [...processedRemoteCommands].slice(-200)
    const body = JSON.stringify(snapshot)
    const timestamp = String(Math.floor(Date.now() / 1000))
    const signature = createHmac('sha256', config.token).update(timestamp + '.' + body).digest('hex')
    const response = await postWorkspaceSnapshot(target, {
      Accept: 'application/json',
      'Content-Type': 'application/json',
      'Content-Length': Buffer.byteLength(body),
      'X-DSH-Push-Timestamp': timestamp,
      'X-DSH-Push-Signature': signature,
    }, body)
    const responseText = await response.text()
    if (!response.ok) {
      const message = responseText.replace(/\s+/g, ' ').slice(0, 240)
      throw new Error('HTTP ' + response.status + (message ? ': ' + message : ''))
    }
    let result = null
    try { result = JSON.parse(responseText) } catch (e) { result = null }
    await deliverRemoteCommands(ctx, result?.commands)
    if (workspacePushFailures) console.info('[dsh-activity-tracker] 工作区远端推送已恢复')
    workspacePushFailures = 0
  } catch (e) {
    workspacePushFailures++
    if (workspacePushFailures === 1 || workspacePushFailures % 30 === 0) {
      console.warn('[dsh-activity-tracker] 工作区推送失败：' + String((e && e.message) || e))
    }
...
export function apply(ctx) {
  ...
  void pushWorkspaceStatus(ctx)
  const pushInterval = setInterval(() => { void pushWorkspaceStatus(ctx) }, dashboardConfig().intervalSeconds * 1000)
  ...
  const disposer = webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (url.pathname === API + '/workspaces') {
        ...
        if (!expected) return sendJson(res, 503, { ok: false, error: 'dashboard integration is not configured' })
        if (!sameSecret(req.headers['x-dsh-dashboard-token'], expected)) return sendJson(res, 403, { ok: false, error: 'forbidden' })
        return sendJson(res, 200, buildWorkspaceStatus())
      }
      if (req.headers['x-dsh-activity'] !== '1') return sendJson(res, 403, { ok: false, error: 'forbidden' })
      try {
        if (url.pathname === API + '/dashboard/pair') {
          if (req.method !== 'POST') return sendJson(res, 405, { ok: false, error: 'method not allowed' })
          const body = await readJsonBody(req)
          const code = String(body.code || '').trim()
          const pushUrl = String(body.pushUrl || '').trim()
          if (!/^\d{6}$/.test(code)) return sendJson(res, 400, { ok: false, error: '配对码必须是 6 位数字' })
          return sendJson(res, 200, await pairDashboard(pushUrl, code, body.allowDetails === true))
        }
        if (url.pathname === API + '/dashboard/config') {
          if (req.method !== 'GET') return sendJson(res, 405, { ok: false, error: 'method not allowed' })
          const config = dashboardConfig()
          return sendJson(res, 200, { ok: true, configured: Boolean(config.token && config.pushUrl), pushUrl: config.pushUrl, authorizedWorkspaces: config.authorizedWorkspaces, intervalSeconds: config.intervalSeconds })
        }
  ...
  ctx.on('dispose', () => { clearInterval(pushInterval); disposer() })
}
