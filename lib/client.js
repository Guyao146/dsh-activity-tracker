...
if (React !== null) {
  ...
  const DEFAULT_FILTERS = { project: 'all', scope: '7d', customStart: '', customEnd: '', page: 0 }
  ...
  function ActivityPanel(props) {
    ...
    React.useEffect(function () {
      apiFetch(HOST_API + '/ui/config').then(function (got) {
        ...
        if (value.filters) {
          ...
          setProject(next.project); setScope(next.scope); setCustomStart(next.customStart); setCustomEnd(next.customEnd); setRangePage(next.page)
    ...
    React.useEffect(function () {
      if (uiLoaded) writeFilters({ project: project, scope: scope, customStart: customStart, customEnd: customEnd, page: rangePage })
    }, [project, scope, customStart, customEnd, rangePage, uiLoaded])
    React.useEffect(function () {
    ...
    React.useEffect(function () {
      if (project !== 'all' && projects.every(function (p) { return p.key !== project })) setProject('all')
    }, [data, project])
    ...
    const header = el('div', { className: 'dsat-header' },
      ...
      activeTab === 'activity' ? projectSelect : null, activeTab === 'activity' ? rangeControls : null,
      tabButtons,
...
.dsat-range-controls{display:flex;align-items:center;gap:6px;flex-wrap:wrap;}
...
@container dsat-panel (max-width:760px){
  ...
  .dsat-header{align-items:flex-start}.dsat-range-controls{order:3;width:100%}
  .dsat-header>.dsat-select{order:3;width:100%}
  .dsat-tabs{order:4}.dsat-header>.dsat-mini{order:4}.dsat-close{order:4}
