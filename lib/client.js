...
if (React !== null) {
  ...
  const DEFAULT_LAYOUT = {
    ...
    modules: {
      ...
      legend: { enabled: true, size: 'small' }, hourly: { enabled: true, size: 'large' },
      tokens: { enabled: true, size: 'large' }, timeline: { enabled: true, size: 'large' }, daily: { enabled: true, size: 'large' },
    },
    order: ['summary', 'heatmap', 'legend', 'hourly', 'tokens', 'timeline', 'daily'],
  }
  function readLayout() {
    try {
  ...
  function ActivityPanel(props) {
    ...
    const activityBody = loading
      ...
      : error
        ...
        : (!data || !Object.keys(daysAll).length)
          ...
          : (function () {
              const moduleContent = {
                summary: el(SummaryCards, { days: scopedDays, sessions: scopedSessions }),
                heatmap: el(Heatmap, { days: daysAll, selDate: selDate, onPick: function (k) { setSelDate(k); setEvLimit(250) } }),
                legend: legend,
                hourly: el('div', null,
                ...
                daily: el(DailyTable, { days: daysAll, selDate: selDate, onPick: function (k) { setSelDate(k); setEvLimit(250) } }),
              }
    ...
    return el('div', { className: 'dsat-root' + (variant === 'overlay' ? ' dsat-overlay' : ' dsat-tab'), style: variant === 'overlay' ? { width: 'min(' + Number(layout.width || 1020) + 'px,calc(100vw - 32px))' } : null },
      ...
      header, account ? el(AccountSummary, { account: account }) : null, panelBody)
  }
