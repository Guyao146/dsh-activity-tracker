  ...
  function PricingSettings(props) {
    ...
    return el('div', { className: 'dsat-body' },
      ...
      el('div', { className: 'dsat-settings' },
        ...
        el('label', { className: 'dsat-field' }, 'Sub2API 用户 JWT', el('input', { className: 'dsat-input', type: 'password', value: apiKey, placeholder: props.config && props.config.configured ? '已保存（留空保持不变）' : '粘贴登录后获取的 access_token/JWT', onChange: function (e) { setApiKey(e.target.value) } })),
      ...
      el('div', { className: 'dsat-settings-help' }, 'Sub2API 接口：/api/v1/channels/available。此接口要求 Sub2API 用户 JWT（登录后返回的 access_token），不是管理员 API Key；可填写纯 JWT，也可填写带 Bearer 前缀的值。Token 只保存在 DSH 宿主机的配置文件中，不会返回给浏览器或写入统计结果。留空分组 ID 时，同一模型存在多个分组会自动选择当前价格最低的分组。'),
