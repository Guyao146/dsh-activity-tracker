...
let uiSaveTimer = 0
let pendingUiSave = {}
function scheduleUiSave(part) {
  pendingUiSave = Object.assign({}, pendingUiSave, part || {})
  window.clearTimeout(uiSaveTimer)
  uiSaveTimer = window.setTimeout(function () {
    const body = pendingUiSave
    pendingUiSave = {}
    apiFetch(HOST_API + '/ui/config', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
  }, 350)
}
...
if (React !== null) {
  ...
  function SummaryCards(props) {
    ...
    if (user.balance !== undefined) cards.push({ label: '账户余额', value: '$' + Number(user.balance || 0).toFixed(2), color: '#4c7dff', title: 'Sub2API 当前账户余额' })
  ...
  function Heatmap(props) {
    ...
    const startKey = props.startKey || addDays(today, -29)
    const endKey = props.endKey || today
    const start = new Date(startKey + 'T12:00:00')
    const end = new Date(endKey + 'T12:00:00')
    const first = addDays(startKey, -((start.getDay() + 6) % 7))
    const last = addDays(endKey, 6 - ((end.getDay() + 6) % 7))
    ...
    const weeks = []
    let cursor = first
    const weekCount = Math.max(1, Math.round((new Date(last + 'T12:00:00') - new Date(first + 'T12:00:00')) / 86400000 / 7) + 1)
    for (let w = 0; w < weekCount; w++) {
      ...
      for (let dow = 0; dow < 7; dow++) {
        ...
        col.push(el('div', {
          ...
          title: tip,
          onClick: function () { if (onPick && key >= startKey && key <= endKey && key <= today) onPick(key) },
        }))
    ...
    return el('div', { className: 'dsat-heat-wrap' },
      el('div', { className: 'dsat-section-label' }, props.label || ('活跃热力图（' + startKey + ' 至 ' + endKey + '，颜色越深 token 消耗越多，点击看当日明细）')),
      ...
      props.legend ? el('div', { className: 'dsat-heat-legend-events' }, props.legend) : null)
  }
  ...
  function DailyTable(props) {
    ...
    const keys = allKeys.slice(0, props.limit && props.limit > 0 ? props.limit : allKeys.length)
    ...
    return el('div', { className: 'dsat-table-wrap' },
      el('div', { className: 'dsat-section-label' }, props.label || '每日汇总（点击行查看当日明细）'),
  ...
  const DEFAULT_LAYOUT = {
    width: 1020,
    modules: {
      summary: { enabled: true, size: 'large' }, heatmap: { enabled: true, size: 'large' },
      legend: { enabled: false, size: 'small' }, hourly: { enabled: true, size: 'large' },
      tokens: { enabled: true, size: 'large' }, timeline: { enabled: true, size: 'large' }, daily: { enabled: true, size: 'large' },
    },
    order: ['summary', 'heatmap', 'hourly', 'tokens', 'timeline', 'daily'],
  }
  const DEFAULT_FILTERS = { project: 'all', session: 'all', scope: '7d', customStart: '', customEnd: '', page: 0 }
  function readFilters() { try { return Object.assign({}, DEFAULT_FILTERS, JSON.parse(localStorage.getItem('dsat-filters') || '{}')) } catch (e) { return Object.assign({}, DEFAULT_FILTERS) } }
  function writeFilters(filters) { try { localStorage.setItem('dsat-filters', JSON.stringify(filters)) } catch (e) {}; scheduleUiSave({ filters: filters }) }
  function readLayout() {
    try {
      const saved = JSON.parse(localStorage.getItem('dsat-layout') || '{}')
      return {
        width: Number(saved.width) >= 620 ? Number(saved.width) : DEFAULT_LAYOUT.width,
        modules: Object.keys(DEFAULT_LAYOUT.modules).reduce(function (out, id) {
          out[id] = Object.assign({}, DEFAULT_LAYOUT.modules[id], (saved.modules && saved.modules[id]) || {})
          return out
        }, {}),
        order: Array.isArray(saved.order) ? saved.order.filter(function (id) { return DEFAULT_LAYOUT.order.indexOf(id) >= 0 }) .concat(DEFAULT_LAYOUT.order.filter(function (id) { return saved.order.indexOf(id) < 0 })) : DEFAULT_LAYOUT.order.slice(),
      }
    } catch (e) { return { width: DEFAULT_LAYOUT.width, modules: Object.assign({}, DEFAULT_LAYOUT.modules), order: DEFAULT_LAYOUT.order.slice() } }
  }
  function writeLayout(layout) { try { localStorage.setItem('dsat-layout', JSON.stringify(layout)) } catch (e) {}; scheduleUiSave({ layout: layout }) }
  function moduleClass(layout, id) { const m = layout.modules[id] || {}; return 'dsat-module dsat-size-' + (m.size || 'large') + (m.enabled === false ? ' dsat-module-hidden' : '') }
  const MODULE_LABELS = { summary: '概览卡片', heatmap: '活跃热力图', legend: '事件图例', hourly: '24 小时活动', tokens: '24 小时 Token', timeline: '当日时间线', daily: '每日汇总' }

  function ModuleMenu(props) {
    const [open, setOpen] = React.useState(false)
    function choose(value) { setOpen(false); if (props.onChoose) props.onChoose(value) }
    return el('div', { className: 'dsat-module-menu' },
      el('button', { className: 'dsat-module-menu-btn', title: '模块设置', onClick: function (e) { e.stopPropagation(); setOpen(!open) } }, '⋯'),
      open ? el('div', { className: 'dsat-module-menu-pop', onClick: function (e) { e.stopPropagation() } },
        el('button', { onClick: function () { choose('small') } }, '小'),
        el('button', { onClick: function () { choose('medium') } }, '中'),
        el('button', { onClick: function () { choose('large') } }, '大'),
        el('button', { className: 'dsat-menu-close', onClick: function () { choose('close') } }, '关闭模块')) : null)
  }
  ...
  function AccountSummary(props) {
    ...
    const items = subs.subscriptions || []
    return el('div', { className: 'dsat-account-summary' },
      el('div', { className: 'dsat-account-brand' }, '中转站：' + (account.siteName || 'Sub2API')),
      user.email || user.username ? el('span', { className: 'dsat-account-user' }, user.email || user.username) : null,
      user.balance !== undefined ? el('span', { className: 'dsat-account-chip' }, '余额 $' + Number(user.balance || 0).toFixed(2)) : null,
      items.map(function (s) {
        const quotas = [
          ['日', s.daily_used_usd, s.daily_limit_usd],
          ['周', s.weekly_used_usd, s.weekly_limit_usd],
          ['月', s.monthly_used_usd, s.monthly_limit_usd],
        ].filter(function (x) { return Number(x[2]) > 0 }).map(function (x) { return x[0] + ' ' + fmtMoney(x[1]) + ' / ' + fmtMoney(x[2]) })
        return el('span', { className: 'dsat-account-chip dsat-sub-chip', key: s.id || s.group_name },
          (s.group_name || '订阅') + (quotas.length ? ' · ' + quotas.join(' · ') : ' · 有效订阅'))
      }))
  }
  ...
  function LayoutSettings(props) {
    ...
    function update(next) { setLayout(next); writeLayout(next); if (props.onChange) props.onChange(next) }
    function toggle(id) { const next = Object.assign({}, layout, { modules: Object.assign({}, layout.modules, { [id]: Object.assign({}, layout.modules[id], { enabled: !layout.modules[id].enabled }) }) }); update(next) }
    function size(id, value) { const next = Object.assign({}, layout, { modules: Object.assign({}, layout.modules, { [id]: Object.assign({}, layout.modules[id], { size: value }) }) }); update(next) }
    function move(id, delta) { const order = layout.order.slice(), at = order.indexOf(id), to = at + delta; if (at < 0 || to < 0 || to >= order.length) return; const x = order[at]; order[at] = order[to]; order[to] = x; update(Object.assign({}, layout, { order: order })) }
    function dropOn(target) { if (!dragged || dragged === target) return; const order = layout.order.filter(function (id) { return id !== dragged }); const at = order.indexOf(target); order.splice(at < 0 ? order.length : at, 0, dragged); update(Object.assign({}, layout, { order: order })); setDragged(null) }
    return el('div', { className: 'dsat-body' },
      el('div', { className: 'dsat-section-label' }, '总设置 · 仪表盘布局'),
      el('label', { className: 'dsat-field' }, '统计面板宽度（px）', el('input', { className: 'dsat-input', type: 'range', min: 620, max: 1500, value: layout.width, onChange: function (e) { update(Object.assign({}, layout, { width: Number(e.target.value) })) } })),
      el('div', { className: 'dsat-settings-help' }, '拖动宽度滑块调整面板；每个模块可关闭、调整大小和排序。设置同时保存在浏览器与 DSH 宿主机，重启后会自动恢复。'),
      el('div', { className: 'dsat-layout-list' }, layout.order.map(function (id, index) {
        const m = layout.modules[id]
        return el('div', { className: 'dsat-layout-row' + (dragged === id ? ' dsat-layout-dragging' : ''), key: id, draggable: true,
            onDragStart: function () { setDragged(id) }, onDragOver: function (e) { e.preventDefault() }, onDrop: function () { dropOn(id) }, onDragEnd: function () { setDragged(null) } },
          el('input', { type: 'checkbox', checked: m.enabled !== false, onChange: function () { toggle(id) } }),
          el('span', { className: 'dsat-layout-name' }, MODULE_LABELS[id]),
          el('select', { className: 'dsat-select', value: m.size || 'large', onChange: function (e) { size(id, e.target.value) } },
            el('option', { value: 'small' }, '小'), el('option', { value: 'medium' }, '中'), el('option', { value: 'large' }, '大')),
          el('button', { className: 'dsat-mini', disabled: index === 0, onClick: function () { move(id, -1) } }, '↑'),
          el('button', { className: 'dsat-mini', disabled: index === layout.order.length - 1, onClick: function () { move(id, 1) } }, '↓'))
      })))
  }

  function DashboardSettings() {
    const [baseUrl, setBaseUrl] = React.useState('https://life.mcylyr.cn/config.php')
    const [code, setCode] = React.useState('')
    const [allowDetails, setAllowDetails] = React.useState(false)
    const [current, setCurrent] = React.useState(null)
    const [message, setMessage] = React.useState(null)
    const [busy, setBusy] = React.useState(false)
    React.useEffect(function () {
      apiFetch(HOST_API + '/dashboard/config').then(function (got) {
        if (got.data && got.data.ok) {
          setCurrent(got.data)
          if (got.data.pushUrl) setBaseUrl(String(got.data.pushUrl).replace(/[?&]action=workspace-push$/, ''))
          setAllowDetails(got.data.authorizedWorkspaces === '*')
        }
      })
    }, [])
    function pair() {
      setBusy(true); setMessage(null)
      apiFetch(HOST_API + '/dashboard/pair', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pushUrl: baseUrl, code: code, allowDetails: allowDetails }) }).then(function (got) {
        setBusy(false)
        if (!got.data || !got.data.ok) { setMessage({ error: (got.data && got.data.error) || ('HTTP ' + got.status) }); return }
        setCode(''); setCurrent({ ok: true, configured: true, pushUrl: got.data.pushUrl, authorizedWorkspaces: got.data.authorizedWorkspaces }); setMessage({ text: 'DSH 已连接。后台会自动开始推送工作区状态。' })
      })
    }
    function unpair() {
      setCurrent(null); setCode(''); setMessage({ text: '请输入看板新生成的验证码以更新本机连接。' })
    }
    return el('div', { className: 'dsat-dashboard-settings' },
      el('div', { className: 'dsat-dashboard-heading' }, el('div', { className: 'dsat-section-label' }, '生活看板连接'), el('span', { className: 'dsat-plugin-version' }, '插件 v' + PLUGIN_VERSION)),
      current && current.configured ? el('div', { className: 'dsat-dashboard-connected' },
        el('span', { className: 'dsat-dashboard-dot' }), el('div', null, el('b', null, '已连接生活看板'), el('small', null, current.authorizedWorkspaces === '*' ? '已授权查看所有工作区会话' : '仅推送工作区汇总，不上传会话详情')),
        el('button', { className: 'dsat-mini', disabled: busy, onClick: unpair }, '更新连接')) : null,
      el('label', { className: 'dsat-field' }, '看板地址', el('input', { className: 'dsat-input', value: baseUrl, placeholder: 'https://life.mcylyr.cn/config.php', onChange: function (e) { setBaseUrl(e.target.value) } })),
      el('label', { className: 'dsat-field' }, '六位配对码', el('input', { className: 'dsat-input dsat-pair-code-input', inputMode: 'numeric', maxLength: 6, value: code, placeholder: '在看板设置中生成', onChange: function (e) { setCode(e.target.value.replace(/\D/g, '').slice(0, 6)) } })),
      el('label', { className: 'dsat-checkbox-field' }, el('input', { type: 'checkbox', checked: allowDetails, onChange: function (e) { setAllowDetails(Boolean(e.target.checked)) } }), el('span', null, el('b', null, '允许查看会话详情'), el('small', null, '看板管理员可查看会话记录并向运行中的 DSH 会话发送消息'))),
      el('button', { className: 'dsat-mini dsat-primary', disabled: busy || code.length !== 6 || !baseUrl, onClick: pair }, busy ? '正在配对…' : '输入验证码并连接'),
      el('div', { className: 'dsat-settings-help' }, '配对码 5 分钟内有效且只能使用一次。插件只保存 HTTPS 看板地址、推送令牌和权限开关，不会保存验证码。'),
      message ? el('div', { className: message.error ? 'dsat-error' : 'dsat-success' }, message.error || message.text) : null)
  }

  function TotalSettings(props) {
    return el('div', { className: 'dsat-settings-stack' },
      el(DashboardSettings, null),
      el(PricingSettings, { config: props.config, onSaved: props.onSaved }),
      el(LayoutSettings, { layout: props.layout, onChange: props.onLayoutChange }))
  }
  ...
  function ActivityPanel(props) {
    ...
    const [project, setProject] = React.useState(savedFilters.project)
    const [session, setSession] = React.useState(savedFilters.session)
    const [scope, setScope] = React.useState(savedFilters.scope)
    const [customStart, setCustomStart] = React.useState(savedFilters.customStart)
    const [customEnd, setCustomEnd] = React.useState(savedFilters.customEnd)
    const [rangePage, setRangePage] = React.useState(savedFilters.page)
    ...
    function startResize(e) {
      ...
      e.preventDefault()
      const start = { x: e.clientX, width: Number(layout.width || 1020) }
      resizeRef.current = start
      const move = function (ev) {
        if (!resizeRef.current) return
        const nextLayout = Object.assign({}, layout, { width: Math.max(620, Math.min(1500, Math.round(start.width - (ev.clientX - start.x)))) })
        setLayout(nextLayout); writeLayout(nextLayout)
      }
      const stop = function () { resizeRef.current = null; window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', stop) }
      window.addEventListener('pointermove', move); window.addEventListener('pointerup', stop)
    }
    function dropDashboardModule(e, target) {
      e.preventDefault()
      const source = e.dataTransfer && e.dataTransfer.getData('text/dsat-module')
      if (!source || source === target) return
      const order = layout.order.filter(function (id) { return id !== source })
      const at = order.indexOf(target)
      order.splice(at < 0 ? order.length : at, 0, source)
      const next = Object.assign({}, layout, { order: order })
      setLayout(next); writeLayout(next)
    }
    ...
    React.useEffect(function () {
      apiFetch(HOST_API + '/ui/config').then(function (got) {
        const value = got.data || {}
        if (value.layout) { const next = Object.assign({}, readLayout(), value.layout); setLayout(next); try { localStorage.setItem('dsat-layout', JSON.stringify(next)) } catch (e) {} }
        else scheduleUiSave({ layout: layout })
        if (value.filters) {
          const next = Object.assign({}, DEFAULT_FILTERS, value.filters)
          setProject(next.project); setSession(next.session); setScope(next.scope); setCustomStart(next.customStart); setCustomEnd(next.customEnd); setRangePage(next.page)
          try { localStorage.setItem('dsat-filters', JSON.stringify(next)) } catch (e) {}
        }
        setUiLoaded(true)
      })
    }, [])
    React.useEffect(function () {
      if (uiLoaded) writeFilters({ project: project, session: session, scope: scope, customStart: customStart, customEnd: customEnd, page: rangePage })
    }, [project, session, scope, customStart, customEnd, rangePage, uiLoaded])
    React.useEffect(function () {
      if (!pricingInfo || !pricingInfo.config || !pricingInfo.config.loggedIn) return
      apiFetch(HOST_API + '/pricing/account').then(function (got) {
        if (got.data && got.data.ok) setPricingInfo(got.data)
      })
    }, [pricingInfo && pricingInfo.config && pricingInfo.config.loggedIn, pricingRefresh])
    ...
    React.useEffect(function () {
      ...
      let alive = true
      setDayLoading(true)
      const q = '?date=' + encodeURIComponent(selDate) + (project !== 'all' ? '&project=' + encodeURIComponent(project) : '') + (session !== 'all' ? '&session=' + encodeURIComponent(session) : '')
    ...
    }, [data, selDate, project, session])

    const projects = (data && data.projects) || []
    const sessionsForProject = ((data && data.sessions) || []).filter(function (s) { return project === 'all' || s.projectKey === project })
    React.useEffect(function () {
      if (project !== 'all' && projects.every(function (p) { return p.key !== project })) { setProject('all'); setSession('all') }
    }, [data, project])
    React.useEffect(function () {
      if (session !== 'all' && sessionsForProject.every(function (s) { return s.id !== session })) setSession('all')
    }, [data, project, session])
    const daysAll = data ? ((session !== 'all' ? (data.bySession[session] || { days: {} }) : (project === 'all' ? data.all : (data.byProject[project] || { days: {} }))).days || {}) : {}
    // scope 过滤
    let rangeStart = data && data.range && data.range.first ? data.range.first : addDays(today, -29)
    let rangeEnd = today
    let customTooLong = false
    if (scope === 'today') rangeStart = today
    else if (scope === '7d') rangeStart = addDays(today, -6)
    else if (scope === '15d') rangeStart = addDays(today, -14)
    else if (scope === '30d') rangeStart = addDays(today, -29)
    else if (scope === 'all') rangeStart = data && data.range && data.range.first && data.range.first < addDays(today, -181) ? data.range.first : addDays(today, -181)
    else if (scope === 'custom' && /^\d{4}-\d{2}-\d{2}$/.test(customStart) && /^\d{4}-\d{2}-\d{2}$/.test(customEnd) && customStart <= customEnd) {
      const totalDays = Math.floor((new Date(customEnd + 'T12:00:00') - new Date(customStart + 'T12:00:00')) / 86400000) + 1
      customTooLong = totalDays > 30
      rangeStart = addDays(customStart, Math.max(0, rangePage) * 30)
      rangeEnd = rangeStart <= customEnd ? (customTooLong ? (addDays(rangeStart, 29) < customEnd ? addDays(rangeStart, 29) : customEnd) : customEnd) : customEnd
    }
    const cutoff = rangeStart
    const scopedDays = {}
    for (const k of Object.keys(daysAll)) if (k >= rangeStart && k <= rangeEnd) scopedDays[k] = daysAll[k]
    React.useEffect(function () {
      if (selDate && (selDate < rangeStart || selDate > rangeEnd)) setSelDate(rangeEnd)
    }, [rangeStart, rangeEnd])
    ...
    if (data) {
      ...
      for (const s of data.sessions) {
        ...
        if (session !== 'all' && s.id !== session) continue
        if ((s.lastAt || s.createdAt) < new Date(rangeStart + 'T00:00:00').getTime() || (s.lastAt || s.createdAt) > new Date(rangeEnd + 'T23:59:59').getTime()) continue
        scopedSessions++
      }
    }
    ...
    const projectSelect = el('select', {
      className: 'dsat-select dsat-filter-select', value: project,
      onChange: function (e) { setProject(e.target && e.target.value || 'all'); setSession('all'); setEvLimit(250) },
    },
    ...
    const sessionSelect = el('select', {
      className: 'dsat-select dsat-filter-select', value: session,
      onChange: function (e) { setSession(e.target && e.target.value || 'all'); setEvLimit(250) },
    },
      el('option', { value: 'all' }, '全部会话'),
      sessionsForProject.map(function (s) {
        const label = s.title ? s.title : ('会话 ' + s.short)
        return el('option', { value: s.id, key: s.id }, (project === 'all' ? s.project + ' · ' : '') + label + '（' + s.turns + ' 轮）')
      }))

    const scopeSelect = el('select', {
      className: 'dsat-select dsat-filter-select', value: scope,
      onChange: function (e) { setScope(e.target && e.target.value || '7d'); setRangePage(0) },
    },
      el('option', { value: 'today' }, '今日'),
      el('option', { value: '7d' }, '近 7 天'),
      el('option', { value: '15d' }, '近 15 天'),
      el('option', { value: '30d' }, '近 30 天'),
      el('option', { value: 'custom' }, '自定义天'),
      el('option', { value: 'all' }, '全部'))

    const rangeControls = el('div', { className: 'dsat-range-controls' },
      scopeSelect,
      scope === 'custom' ? el('input', { className: 'dsat-input dsat-date-input', type: 'date', value: customStart, onChange: function (e) { setCustomStart(e.target.value); setRangePage(0) } }) : null,
      scope === 'custom' ? el('span', null, '至') : null,
      scope === 'custom' ? el('input', { className: 'dsat-input dsat-date-input', type: 'date', value: customEnd, onChange: function (e) { setCustomEnd(e.target.value); setRangePage(0) } }) : null,
      customTooLong ? el('button', { className: 'dsat-mini', disabled: rangePage <= 0, onClick: function () { setRangePage(rangePage - 1) } }, '上一段') : null,
      customTooLong ? el('button', { className: 'dsat-mini', disabled: rangeEnd >= customEnd, onClick: function () { setRangePage(rangePage + 1) } }, '下一段') : null)

    const filterBar = el('div', { className: 'dsat-filter-bar' }, projectSelect, sessionSelect, rangeControls)

    const pricingConfig = pricingInfo ? Object.assign({}, pricingInfo.config || {}, { lastSync: pricingInfo.lastSync, error: pricingInfo.error, account: pricingInfo.account }) : null
    const tabButtons = el('div', { className: 'dsat-tabs' },
      ...
      el('button', { className: 'dsat-tab-btn' + (activeTab === 'settings' ? ' dsat-tab-active' : ''), onClick: function () { setActiveTab('settings') } }, '总设置'))
    const header = el('div', { className: 'dsat-header' },
      el('span', { className: 'dsat-title' }, '\uD83D\uDCCA dsh 活动统计 · v' + PLUGIN_VERSION),
      data ? el('span', { className: 'dsat-sub' },
        (data.range && data.range.last) ? '最近活动 ' + data.range.last : '暂无活动',
        data.tz ? ' · ' + data.tz : '') : null,
      activeTab === 'activity' ? null : el('span', { style: { flex: '1 1 auto' } }),
      activeTab === 'activity' ? filterBar : null,
      tabButtons,
    ...
    const activityBody = loading
      ...
      : error
        ...
        : (!data || !Object.keys(daysAll).length)
          ...
          : (function () {
              const moduleContent = {
                summary: el(SummaryCards, { days: scopedDays, sessions: scopedSessions, account: pricingInfo && pricingInfo.account }),
                heatmap: el(Heatmap, { days: daysAll, startKey: rangeStart, endKey: rangeEnd, legend: legend, selDate: selDate, onPick: function (k) { setSelDate(k); setEvLimit(250) } }),
                hourly: el('div', null,
                  el('div', { className: 'dsat-day-head' },
                    ...
                    dayCounts ? el('span', { className: 'dsat-day-sum' }, KIND_ORDER.map(function (k) { return dayCounts[k] ? KINDS[k].label + ' ' + dayCounts[k] : null }).filter(Boolean).join(' · ') || '当天无活动') : null,
                    selDay ? el('span', { className: 'dsat-day-sum dsat-td-muted' }, ' · 入 ' + fmtK(selDay.ti || 0) + ' / 出 ' + fmtK(selDay.to || 0) + ' / 缓存 ' + fmtK(selDay.tc || 0)) : null),
                  el('div', { className: 'dsat-section-label' }, '24 小时活动分布（按类型分色）'), el(HourChart, { hours: selDay ? selDay.hours : [] })),
                tokens: el('div', null, el('div', { className: 'dsat-section-label' }, '24 小时 token 消耗（输入+输出+缓存读）'), el(TokenHourChart, { hours: selDay ? selDay.hours : [] })),
                timeline: el('div', null, el('div', { className: 'dsat-section-label' }, dayLoading ? '当日时间线（加载中…）' : '当日时间线'), el(EventList, { dayData: dayData, limit: evLimit, onMore: function () { setEvLimit(0) } })),
                daily: el(DailyTable, { days: scopedDays, limit: scope === 'all' ? 0 : 31, label: scope === 'all' ? '每日汇总（全部活跃日，点击行查看当日明细）' : '每日汇总（当前范围内活跃日，点击行查看当日明细）', selDate: selDate, onPick: function (k) { setSelDate(k); setEvLimit(250) } }),
              }
              return el('div', { className: 'dsat-body dsat-dashboard-grid' }, layout.order.map(function (id) {
                return layout.modules[id] && layout.modules[id].enabled !== false
                  ? el('section', { className: moduleClass(layout, id), key: id, draggable: true,
                      onDragStart: function (e) { if (e.dataTransfer) { e.dataTransfer.setData('text/dsat-module', id); e.dataTransfer.effectAllowed = 'move' } },
                      onDragOver: function (e) { e.preventDefault() }, onDrop: function (e) { dropDashboardModule(e, id) } },
                      el(ModuleMenu, { onChoose: function (value) {
                        const nextModules = Object.assign({}, layout.modules, { [id]: Object.assign({}, layout.modules[id], value === 'close' ? { enabled: false } : { size: value }) })
                        const next = Object.assign({}, layout, { modules: nextModules }); setLayout(next); writeLayout(next)
                      } }), moduleContent[id])
                  : null
              }), el('div', { className: 'dsat-footer dsat-grid-full' }, '数据源：~/.dsh/sessions（session.jsonl.zstd）· ' + (data.sessions ? data.sessions.length : 0) + ' 个会话 · 时区 ' + (data.tz || '本地')))
            })()

    const panelBody = activeTab === 'cost'
      ? el(CostPanel, { refreshKey: pricingRefresh })
      : activeTab === 'settings'
        ? el(TotalSettings, { config: pricingConfig, layout: layout, onLayoutChange: setLayout, onSaved: function (next) { setPricingInfo(next); setPricingRefresh(pricingRefresh + 1) } })
        : activityBody
    const account = pricingInfo && pricingInfo.account
    return el('div', { className: 'dsat-root' + (variant === 'overlay' ? ' dsat-overlay' : ' dsat-tab'), style: variant === 'overlay' ? { width: 'min(' + Number(layout.width || 1020) + 'px,calc(100vw - 32px))' } : null },
      variant === 'overlay' ? el('div', { className: 'dsat-resize-handle', title: '拖动调整宽度', onPointerDown: startResize }) : null,
      header, panelBody)
  }
...
// ---------------- 样式 ----------------
const CSS = `
.dsat-root{display:flex;flex-direction:column;font-size:13px;line-height:1.5;color:inherit;min-height:0;container-type:inline-size;container-name:dsat-panel;}
.dsat-tab{height:100%;}
...
.dsat-resize-handle{position:absolute;left:-5px;top:0;bottom:0;width:10px;cursor:ew-resize;z-index:3;}
.dsat-resize-handle:hover{background:rgba(76,125,255,.25);}
@media (prefers-color-scheme: dark){.dsat-overlay{background:#16181d;color:#e6e6e6;}}
.dsat-header{position:relative;display:flex;align-items:center;gap:8px;padding:10px 52px 10px 14px;border-bottom:1px solid rgba(127,127,127,.25);flex:0 0 auto;flex-wrap:wrap;}
.dsat-filter-bar{display:grid;grid-template-columns:minmax(150px,1fr) minmax(180px,1.2fr) minmax(120px,.7fr);gap:8px;flex:1 1 540px;align-items:start;}
.dsat-range-controls{display:flex;align-items:center;gap:6px;flex-wrap:wrap;min-width:0;}
.dsat-filter-select{box-sizing:border-box;width:100%;height:34px;padding:5px 10px;min-width:0;}
.dsat-range-controls>.dsat-filter-select{flex:1 1 120px;}
.dsat-date-input{width:132px;padding:5px 7px;}
...
.dsat-close{position:absolute;right:12px;top:10px;width:30px;height:30px;border:none;background:transparent;cursor:pointer;font-size:16px;color:inherit;opacity:.7;padding:0;border-radius:6px;z-index:6;}
...
.dsat-cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;}
.dsat-card{min-width:0;border:1px solid rgba(127,127,127,.25);border-radius:10px;padding:8px 12px;background:rgba(127,127,127,.05);}
...
.dsat-settings-stack{display:flex;flex-direction:column;min-height:0;overflow-y:auto;}
.dsat-account-summary{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:8px 14px;border-bottom:1px solid rgba(127,127,127,.2);background:rgba(76,125,255,.06);font-size:11px;}
.dsat-account-brand{font-weight:700;color:#4c7dff;}
.dsat-account-user{opacity:.75;}
.dsat-account-chip{padding:3px 8px;border-radius:999px;background:rgba(127,127,127,.12);font-variant-numeric:tabular-nums;}
.dsat-sub-chip{background:rgba(46,160,67,.12);color:#2ea043;}
.dsat-layout-list{display:flex;flex-direction:column;gap:6px;}
.dsat-layout-row{display:flex;align-items:center;gap:8px;padding:7px 8px;border:1px solid rgba(127,127,127,.2);border-radius:8px;}
.dsat-layout-name{flex:1;}
.dsat-module{position:relative;border:1px solid rgba(127,127,127,.16);border-radius:10px;padding:30px 10px 10px;min-width:0;}
.dsat-module-menu{position:absolute;right:6px;top:4px;z-index:5;}
.dsat-module-menu-btn{width:26px;height:24px;border:none;border-radius:6px;background:rgba(127,127,127,.1);color:inherit;cursor:pointer;font-size:18px;line-height:18px;opacity:.65;}
.dsat-module-menu-btn:hover{opacity:1;background:rgba(127,127,127,.2);}
.dsat-module-menu-pop{position:absolute;right:0;top:27px;display:flex;flex-direction:column;min-width:92px;padding:4px;background:#fff;color:#1f2328;border:1px solid rgba(127,127,127,.35);border-radius:8px;box-shadow:0 6px 20px rgba(0,0,0,.25);}
@media (prefers-color-scheme: dark){.dsat-module-menu-pop{background:#202328;color:#e6e6e6;}}
.dsat-module-menu-pop button{border:none;border-radius:5px;padding:5px 9px;text-align:left;background:transparent;color:inherit;cursor:pointer;font-size:11px;}
.dsat-module-menu-pop button:hover{background:rgba(76,125,255,.15);}
.dsat-module-menu-pop .dsat-menu-close{color:#d64545;border-top:1px solid rgba(127,127,127,.2);margin-top:3px;}
.dsat-module[draggable=true]{cursor:grab;}
.dsat-module[draggable=true]:active{cursor:grabbing;}
.dsat-module-hidden{display:none;}
.dsat-dashboard-grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));align-content:start;}
.dsat-size-small{font-size:11px;grid-column:span 4;}
.dsat-size-small .dsat-card-val{font-size:14px;}
.dsat-size-medium{font-size:12px;grid-column:span 6;}
.dsat-size-large{font-size:13px;grid-column:span 12;}
.dsat-grid-full{grid-column:1/-1;}
@container dsat-panel (max-width:900px){
  .dsat-size-small{grid-column:span 6}.dsat-size-medium{grid-column:span 12}
  .dsat-header>span[style]{display:none}
  .dsat-title{flex:1 1 auto}.dsat-sub{order:2;width:100%}
  .dsat-filter-bar{order:3;grid-template-columns:1fr 1fr;width:100%;flex-basis:100%}
  .dsat-range-controls{grid-column:1/-1}
}
@container dsat-panel (max-width:760px){
  .dsat-size-small,.dsat-size-medium,.dsat-size-large{grid-column:span 12}
  .dsat-header{align-items:flex-start}.dsat-filter-bar{grid-template-columns:1fr;width:100%}
  .dsat-range-controls{grid-column:auto;width:100%}
  .dsat-tabs{order:4}.dsat-header>.dsat-mini{order:4}
  .dsat-cards{grid-template-columns:repeat(2,minmax(0,1fr))}
  .dsat-account-summary{align-items:flex-start}
  .dsat-heat-body,.dsat-hourchart{min-width:0}
}
@container dsat-panel (max-width:520px){
  .dsat-cards{grid-template-columns:1fr}
  .dsat-date-input{width:100%}.dsat-range-controls{align-items:stretch}
  .dsat-table-wrap{overflow-x:auto}.dsat-table{min-width:620px}
}
...
.dsat-dashboard-settings{display:flex;flex-direction:column;gap:10px;max-width:720px;padding:12px;border:1px solid rgba(76,125,255,.28);border-radius:10px;background:rgba(76,125,255,.045);}
.dsat-dashboard-heading{display:flex;align-items:center;justify-content:space-between;gap:12px}.dsat-plugin-version{padding:3px 8px;border-radius:999px;background:rgba(76,125,255,.12);color:#4c7dff;font:600 10px ui-monospace,Consolas,monospace;white-space:nowrap}
.dsat-dashboard-connected{display:flex;align-items:center;gap:9px;padding:9px 10px;border-radius:8px;background:rgba(46,160,67,.1);}.dsat-dashboard-connected>div{display:grid;gap:2px;flex:1;min-width:0;}.dsat-dashboard-connected small{opacity:.72;font-size:11px;}.dsat-dashboard-dot{width:9px;height:9px;border-radius:50%;background:#2ea043;box-shadow:0 0 0 4px rgba(46,160,67,.15);}.dsat-pair-code-input{font:600 18px ui-monospace,Consolas,monospace;letter-spacing:.22em;}
.dsat-checkbox-field{display:flex;align-items:flex-start;gap:8px;font-size:12px;cursor:pointer;}.dsat-checkbox-field input{margin:3px 0 0;accent-color:#4c7dff;}.dsat-checkbox-field span{display:grid;gap:2px;}.dsat-checkbox-field small{opacity:.7;font-size:11px;}
/* 热力图 */
...
.dsat-heat-legend-events{padding:6px 8px;border:1px solid rgba(127,127,127,.18);border-radius:8px;background:rgba(127,127,127,.04);}
/* 图例 */
...
.dsat-pair-code-input{height:48px!important;text-align:center;font:600 24px/1 ui-monospace,Consolas,monospace!important;letter-spacing:.28em!important;color:#4c7dff!important;background:rgba(76,125,255,.06)!important}.dsat-pair-code-input::placeholder{font:400 13px/1 system-ui,sans-serif!important;letter-spacing:0!important;color:inherit;opacity:.48}.dsat-dashboard-settings .dsat-field,.dsat-dashboard-settings .dsat-input:not(.dsat-pair-code-input),.dsat-dashboard-settings .dsat-settings-help{letter-spacing:0}.dsat-dashboard-settings .dsat-settings-help{line-height:1.65}
`
