...
async function apiFetch(path) {
  try {
    const r = await window.fetch(path, { headers: { 'X-DSH-Activity': '1' } })
    let data = null
...
if (React !== null) {
  ...
  function ActivityPanel(props) {
    ...
    const header = el('div', { className: 'dsat-header' },
      ...
      projectSelect, scopeSelect,
      el('button', { className: 'dsat-mini', title: '重新扫描会话数据', onClick: function () { setLoading(true); setTick(tick + 1) } }, '\u21BB 刷新'),
    ...
    return el('div', { className: 'dsat-root' + (variant === 'overlay' ? ' dsat-overlay' : ' dsat-tab') }, header, body)
  }
